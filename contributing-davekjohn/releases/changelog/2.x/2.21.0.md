# v2.21.0 — Alle mixtitels naar één uniek formaat, plus workflow- en toolingherstel

**Datum:** 2026-07-25
**Type:** Minor

> **Correctie 2026-07-26.** Deze note verantwoordde de titel-migratie oorspronkelijk als een
> SEO-verbetering: 77 `<title>`-tags, de `<h1>`, de mix-kaarten en `mix_title` in GA4 zouden
> meebewegen, met een breuk in historische GA4-rapportages als gevolg. **Dat was onjuist.** Het
> `title`-veld uit `src/data/mixes/*.json` wordt nergens gerenderd; elke zichtbare plek bouwt zijn
> tekst op uit de losse velden `color`, `subgenre` en `volume`. De migratie zelf is correct
> uitgevoerd, alleen de verantwoording was fout. De passages hieronder zijn herschreven naar wat de
> wijziging werkelijk is: een dataconsistentie-verbetering. Zie PR #22 voor de onderbouwing.

## Wijzigingen

De grote wijziging zit in de **data**, niet op de site: alle 77 publieke mixen hebben een nieuwe,
unieke `title` gekregen waarin het subgenre meedoet. Waar eerder 63 mixen elf titels deelden, is nu
elke titel uniek. Daarnaast zijn drie mixen gecorrigeerd die een andere frequentie toonden dan hun
eigen data.

De overige twee wijzigingen zijn intern: het werkwijze-document dat per ongeluk was verwijderd is
teruggehaald en op zes punten gecorrigeerd, en de specialists-toolchain draait weer.
...
uitschieters van 11 × `Purple Light (f)` en 9 × `Orange Light (f)`. Als identificatie van een mix was
het veld daarmee waardeloos: je kon er niet mee zien wélke mix je voor je had, en de veldspec eiste
al langer een uniek formaat. Na de migratie is **elke titel uniek**, zonder uitzondering. Dat komt
doordat het subgenre nu in de titel staat: dat is precies het veld dat mixen met dezelfde kleur,
power en frequentie van elkaar scheidt.

De titel zegt daarmee ook inhoudelijk meer: `Blue Full (f)` zei niets over de muziek,
`Liquid Drum & Bass Mix · Blue Full (f) · Vol. 2` wel. Dat is winst voor wie de JSON leest of
onderhoudt — niet voor een bezoeker of een zoekmachine, want dit veld bereikt geen van beide.
...
redirects nodig en er gaat geen enkele bestaande link kapot.

**Wat er verder niets verandert: alles wat een bezoeker ziet.** Het `title`-veld wordt nergens
gerenderd. Elke zichtbare plek bouwt zijn tekst op uit de losse velden:

| Plek | Bron | Vindplaats |
|---|---|---|
| `<h1>` detailpagina | `{color} {subgenre} Mix {volume}` | `src/app/luister/mix/[slug]/page.tsx` |
| `<title>`, og:title, twitter:title | dezelfde velden + `\| DJ Cylow` | idem, `generateMetadata` |
| GA4 `mix_title` | dezelfde velden | idem, de `MixAnalytics`-aanroep |
| mix-kaart op `/luister` | `{color} {subgenre} Mix · {volume}` | `src/components/luister/Playlist.tsx` |
...
Er waren dus ook geen 63 pagina's met een identieke `<title>` of `<h1>`: die waren al uniek, omdat
ze uit de losse velden komen. En `mix_title` in GA4 verandert niet, dus er breekt niets in de
